...
def make_dataloader(cfg):
    ...
    val_transforms = T.Compose([
        ...
        # T.Pad(10),
        T.RandomResizedCrop(size=(256, 128), scale=(0.5, 1)),
        T.ToTensor(),
        T.Normalize(mean=cfg.INPUT.PIXEL_MEAN, std=cfg.INPUT.PIXEL_STD),
        RandomErasing(probability=1, min_area=1/10, max_area=1/3, mode='pixel', max_count=1, device='cpu')
        # RandomErasing(probability=1, min_area=1/5, max_area=1/3, mode='pixel', max_count=1, device='cpu')
    ])

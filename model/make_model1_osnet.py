...
from .backbones.osnet import OSNet, OSBlock, init_pretrained_weights
import copy
...
class Backbone(nn.Module):
    def __init__(self, num_classes, cfg):
        ...
        if model_name == 'osnet':
            self.in_planes = 256
            self.base = OSNet(num_classes,
                                blocks=[OSBlock, OSBlock, OSBlock],
                                layers=[2, 2, 2],
                                channels=[32, 128, 192, 256],
                                loss='softmax')
            print('using osnet as a backbone')
        else:
            print('unsupported backbone! but got {}'.format(model_name))

        if pretrain_choice == 'imagenet':
            init_pretrained_weights(self.base, key='osnet_x0_5')
    ...
    def forward(self, x, x2, x3, label=None):  # label is unused if self.cos_layer == 'no'
        x = self.base(x)
        x2 = self.base(x2)
        x3 = self.base(x3)
        ...
        global_feat2 = global_feat2.view(global_feat.shape[0], -1)  # flatten to (bs, 2048)
        ...
        if self.training:
            if self.cos_layer:
                ...
                cls_score2 = self.classifier_occ1(feat2)
                cls_score3 = self.classifier_occ2(feat3)
            return [cls_score, cls_score2, cls_score3], [global_feat, global_feat2, global_feat3]
        else:
...
class build_transformer(nn.Module):
    ...
    def forward(self, x, x2, x3, label=None, cam_label= None, view_label=None):
        ...
        if self.training:
            ...
            return [cls_score, cls_score_occ1, cls_score_occ2], [global_feat, global_feat2, global_feat3]  # global feature for triplet loss
        else:
            if self.neck_feat == 'after':
